...
- Consumers run `npx skills add Paldom/screenshooter` — the CLI
  ...
  users can also `gh skill install Paldom/screenshooter <skill>
...
gh repo edit Paldom/screenshooter --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/screenshooter/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/screenshooter --add-topic skills-sh

# 4. Verify installability exactly like a consumer
npx skills add Paldom/screenshooter --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
npx skills add Paldom/screenshooter --skill '*' -a claude-code -y
...
(`https://skills.sh/b/Paldom/screenshooter`) linking to the repo's
skills.sh page.
